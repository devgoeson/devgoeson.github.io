...
window.onload = function () {
    ...
    var touchPointNum = 3;
    var mouseFlag = [];    // マウス操作のフラグ
    var mousePosition = [];
    for (var n = 0; n < touchPointNum; ++n) {
        mouseFlag[n] = false;
        mousePosition[n] = [0.0, 0.0];
    }
    ...
    //c.addEventListener("touchcancel", ?????, false);
    ...
    vUniLocation[2] = gl.getUniformLocation(vPrg, 'velocity');

    var vUniLocFluences = [];
    var vUniLocMousePos = [];
    for (var i = 0; i < touchPointNum; ++i) {
        vUniLocFluences[i] = gl.getUniformLocation(vPrg, 'fluence[' + i + ']');
        vUniLocMousePos[i] = gl.getUniformLocation(vPrg, 'mousePos[' + i + ']');
    }
    ...
    function render() {
        ...
        gl.uniform1f(vUniLocation[2], velocity);

        for (var i = 0; i < touchPointNum; ++i) {
            gl.uniform1f(vUniLocFluences[i], mouseFlag[i] ? 0.2 : 0.0);
            gl.uniform2fv(vUniLocMousePos[i], mousePosition[i]);
        }
    ...
    function mouseMove(eve) {
        if (mouseFlag[0]) {
            var cw = c.width;
            var ch = c.height;
            mousePosition[0][0] = (eve.clientX - c.offsetLeft - cw / 2.0) / cw * 2.0;
            mousePosition[0][1] = -(eve.clientY - c.offsetTop - ch / 2.0) / ch * 2.0;
        }
    }
    function touchStart(eve) {
        for (var i = 0; i < touchPointNum; ++i) {
            if (i < eve.touches.length) {
                mouseFlag[i] = true;
            }
            else {
                mouseFlag[i] = false;
    ...
    function touchMove(eve) {
        ...
        for (var n = 0; n < touchPointNum; ++n) {
            if (mouseFlag[n]) {
                var x = eve.targetTouches[n].pageX;
                var y = eve.targetTouches[n].pageY;
                var cw = c.width;
                var ch = c.height;
                mousePosition[n][0] = (x - c.offsetLeft - cw / 2.0) / cw * 2.0;
                mousePosition[n][1] = -(y - c.offsetTop - ch / 2.0) / ch * 2.0;
            }
        }
    }
